...
export function getRandomNormalColor() {
  // For pastel colors, we use higher base values (180-255)
...
export function getRandomWinners(
  ...
  while (
    ...
    winners.length < wishMessages.length
  ) {
...
export function getRandomWinnersWithExclusion(
  randomPool: WishMessage[],
  exclusionPool: WishMessage[]
): WishMessage[] {
  const winners = [];
  const exclusionName = exclusionPool
